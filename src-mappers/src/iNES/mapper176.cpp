...
#define		wramExtended (!!(wram &0x20) && ROM->INES2_SubMapper ==2)
#define		enableFK23    !!(wram &0x40)
...
void	sync() {
	if (wramEnabled || !wramExtended) { // Do not allow disabling simple WRAM (YH602, YH801)
		if (wramExtended) {
			EMU->SetPRG_RAM8(0x4, (wram &0x03) +1);
			EMU->SetPRG_RAM8(0x6, (wram &0x03) +0);
		} else {
	...
	if (ROM->INES_MapperNum ==523) 
		...
		for (int bank =0; bank <8; bank++) {
			...
			if (chrRAM || wramExtended && chrMixed && val <0x08)
...
void	MAPINT	writeReg (int bank, int addr, int val) {
	...
	if ((!wramExtended || enableFK23) && addr &ROM->dipValue) {
...
void	MAPINT	write4800 (int bank, int addr, int val) {
	if (addr &0x800) {
		reg4800 =val;
		sync();
	} else
...
void	MAPINT	reset (RESET_TYPE resetType) {	// Cart detects soft reset, so always do hard reset
	...
	if (ROM->INES2_SubMapper ==5) {
		...
		EMU->SetCPUWriteHandler(0x4, write4800);
	}
}
