...
unsigned char	CPU_RP2A03::MemGet (unsigned int Addr) {
	...
	if (EnableDMA &DMA_SPR) {
		...
		for (int i = 0; i <DMALength; i++) {
			// Quinty performs its Sprite DMA from scanlines 9 to 13, which is where we normally update our inputs (see NES.cpp).
			// Check that scenario here so we do not miss input on that game.
			if (NES::Scanline) {
				NES::Scanline =FALSE;
				if (PPU::PPU[0] && PPU::PPU[0]->SLnum ==10) Controllers::UpdateInput();
			}
			
			MemSet(DMATarget, MemGet((DMAPage << 8) | DMAMiddleAddr | i));
		}
		EnableDMA &=~DMA_IN_SPR;
	}
	RunCycle();
...
__forceinline	void	CPU_RP2A03::IV_UNK (void)
{
...
CPU_RP2A03_PocketNES::CPU_RP2A03_PocketNES(uint8_t* _RAM):
	CPU_RP2A03(_RAM) {
}

CPU_RP2A03_PocketNES::CPU_RP2A03_PocketNES(int _which, int _ramSize, uint8_t* _RAM):
	CPU_RP2A03(_which, _ramSize, _RAM) {
}

uint8_t	CPU_RP2A03_PocketNES::GetOpcode() {
	int addr =PC;
	switch(MemGet(PC)) {
		case 0xFA:
			PRGPointer[addr >>12 &0xF][addr &0xFFF] =0x88; addr++;
			PRGPointer[addr >>12 &0xF][addr &0xFFF] =0xD0; addr++;
			PRGPointer[addr >>12 &0xF][addr &0xFFF] =0xFD; addr++;
			PRGPointer[addr >>12 &0xF][addr &0xFFF] =0xCA; addr++;
			PRGPointer[addr >>12 &0xF][addr &0xFFF] =0xD0; addr++;
			PRGPointer[addr >>12 &0xF][addr &0xFFF] =0xFA; addr++;
			break;
		case 0xFB:
			PRGPointer[addr >>12 &0xF][addr &0xFFF] =0x8D; addr++;
			break;
		case 0xFC:
			PRGPointer[addr >>12 &0xF][addr &0xFFF] =0xE8; addr++;
			break;
		case 0xFF:
			PRGPointer[addr >>12 &0xF][addr &0xFFF] =0xA5; addr++;
			break;
	}
	return CPU_RP2A03::GetOpcode();
}
